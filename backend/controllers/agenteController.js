  ...
  try {
    ...
    if (agents.length === 0) {
      return res
        .status(404)
        .json({
          message: "No se encontraron agentes asociados a tus clientes.",
        });
    }
    ...
    res
      .status(200)
      .json({ message: "Agentes obtenidos correctamente", data: agents });
  } catch (error) {
    res
      .status(500)
...
const getAgentById = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Agente obtenido correctamente", data: agent });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error del servidor", error: error.message });
...
const getAgentsByAdmin = async (req, res) => {
  try {
    if (req.user.rl !== 2) {
    ...
    const { id } = req.user;
    const agents = await Agent.getAgentsByAdmin(id);
    ...
    if (agents.length === 0) {
      return res
        ...
        .json({
          status: "404",
          message: "No se encontraron agentes asociados",
        });
    }
    ...
    console.error("Error en getAgentsByAdmin:", error);
    res.status(500).json({
      status: "500",
