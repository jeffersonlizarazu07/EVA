      ...
      <div className="row d-flex mb-3">
        ...
        <div className="d-grid col-6 col-sm-6 col-md-6 col-lg-6 justify-content-end">
          <button
            ...
            onClick={() => onCreate()}
          >
            <i className="fa fa-plus"></i> {t("addForm")}
          </button>
        </div>
      </div>
      ...
      <table className="table table-hover" id="tableDefault">
        <thead>
          <tr className="table-light tr-table">
            ...
            <th className="col text-center">{t("actions")}</th>
          </tr>
        </thead>
        <tbody>
          {currentRecords.map((item, idx) => (
            <tr key={idx}>
              {header.map((col, i) => (
                <td key={i}>
                  {col === "state" ? (
                    item.state === 1 ? t("clientTable.Active") : t("clientTable.Inactive")
                  ) : (
                  ...
                  <div className="dropdown">
                    ...
                    <ul className="dropdown-menu p-0">
                      <li className="text-start btn-rect">
                        <button className="btn btn-rect" onClick={() => onUpdate(item)}>
                          <i className="fa-solid fa-circle-question"></i> {t("viewBlocks")}
                        </button>
                      </li>
                      <li className="text-start btn-rect">
                        <button
                        ...
                        >
                          <i className="fa-solid fa-edit"></i> {t("edit")}
                        </button>
                      </li>
                      <li className="text-start btn-rect">
                        <button className="btn text-start" style={{ width: "100%" }} onClick={() => onRemove(item)}>
                          <i className="fa-solid fa-power-off"></i> {t("disable")}
                        </button>
                      </li>
                    </ul>
                  </div>
