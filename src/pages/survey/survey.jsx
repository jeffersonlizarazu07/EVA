...
const useIntelligentColorSystem = (color1, color2, isDarkMode) => {
  return useMemo(() => {
    ...
    const textOnPrimary =
      getContrastRatio(primary, "#ffffff") >
      getContrastRatio(primary, "#000000")
        ? "#ffffff"
        : "#000000";
    const textOnSecondary =
      getContrastRatio(secondary, "#ffffff") >
      getContrastRatio(secondary, "#000000")
        ? "#ffffff"
        : "#000000";
    ...
    const adaptedColors = {
      ...
      glassOverlay: alpha(
        ...
        isDarkMode ? 0.05 : 0.95
      ),
      glassOverlayStrong: alpha(
        isDarkMode ? "#ffffff" : "#ffffff",
        isDarkMode ? 0.1 : 0.98
      ),
    };
...
const createIntelligentSwitch = (colors) =>
  styled(Switch)(({ theme }) => ({
    width: 62,
    height: 34,
    padding: 7,
...
export default function Survey() {
  ...
  const colors = useIntelligentColorSystem(
    survey.color_tag1,
    survey.color_tag2,
    theme === "dark"
  );
  ...
  const handleSubmit = async (event) => {
    ...
    const allAnswered = visibleQuestions.every((question) =>
      answers.some(
        (answer) => answer.question_id === question.id && answer.answer !== ""
      )
    );
  ...
  const shouldRenderQuestion = (question) => {
    if (question.id_conditional) {
      const conditionalAnswerObj = answers.find(
        (answer) => answer.question_id === question.id_conditional
      );
  ...
  return (
    <Box
      sx={{
        ...
        } 0%, ${alpha("#ffffff", theme === "dark" ? 0.05 : 0.95)} 100%)`, // TU FONDO GENIAL RESTAURADO
      }}
    >
