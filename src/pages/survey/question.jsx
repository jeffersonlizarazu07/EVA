            ...
            >
              {Array.from({ length: 5 }, (_, i) => i + 1).map((value) => (
                <a
                  ...
                  data-bs-title={`Satisfacción ${value}`}
                >
                  <input
                    type="radio"
                    className="btn-check"
                    name={`satisfaction-${question.id}`}
            ...
            <div className="btn-group" role="group">
              {Array.from({ length: 11 }, (_, i) => i).map((value) => (
                <a
                  key={value}
                  data-bs-toggle="tooltip"
                  data-bs-title={`Probabilidad ${value}`}
                >
                  <input
                    type="radio"
                    className="btn-check"
                    name={`probability-${question.id}`}
                    id={`recomendar${value}`}
                    autoComplete="off"
                    value={value}
                  />
          ...
          <div className="input-group justify-content-center">
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
                name={`yes_no-${question.id}`}
                id="si_no1"
                autoComplete="off"
                value="NO"
              />
              <label className="btn btn-outline-danger" htmlFor="si_no1">
                NO
              </label>
              <input
                type="radio"
                ...
                value="SI"
              />
              <label className="btn btn-outline-success" htmlFor="si_no2">
                SI
              </label>
            </div>
          </div>
          ...
          <ul className="dropdown-menu">
            <li>
              <button
                ...
                onClick={() => openModal(2, question.id, question)}
              >
                Editar
              </button>
            </li>
            <li>
              <button
                ...
                type="button"
                onClick={() =>
                  deleteQuestion(question, config, updateSurveyQuestions, t)
                }
              >
                Eliminar
              </button>
