...
export const ModalBlocks = () => {
  ...
  const handleBlocksOpen = (event) => {
    setSearchTerm(event.target.value);
  };

  const handleBlocksClose = () => {
    setSearchTerm("");
  }

  const handleRecordsPerPageChange = (num) => {
    setRecordsPerPage(num);
    setCurrentPage(1); // Reset to first page when changing records per page
  };

  useEffect(() => {
    // Logic to fetch and filter records based on searchTerm and recordsPerPage
    // Update totalPages based on the filtered records
