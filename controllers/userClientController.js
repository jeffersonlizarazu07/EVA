...
const userClientController = {
    async userClients(req, res) {
        try {
            ...
            if (user_clients.length === 0) {
                return res.status(404).json({ status: '404', message: 'No se encontraron asociaciones' });
            }
            res.json({ status: '200', message: 'Asociaciones obtenidas correctamente', data: user_clients });
        } catch (error) {
    ...
    async userClientByID(req, res) {
        try {
            ...
            res.json({ status: '200', message: 'Asociación obtenida correctamente', data: userClient });
        } catch (error) {
            res.status(500).json({ status: '500', message: 'Error interno del servidor', error });
        }
    },
    ...
    async postUserClient(req, res) {
        try {
            ...
            res.status(500).json({ status: '500', message: 'Error al crear la asociación', error });
        }
    },
    ...
    async putUserClient(req, res) {
        try {
            ...
            res.json({ status: '200', message: 'Asociaciones actualizadas correctamente' });
        } catch (error) {
            res.status(500).json({ status: '500', message: 'Error al actualizar la asociación', error });
        }
    },
    ...
    async deleteUserClient(req, res) {
        try {
            ...
            res.json({ status: '200', message: 'Asociación eliminada correctamente' });
        } catch (error) {
            res.status(500).json({ status: '500', message: 'Error al eliminar la asociación', error });
        }
    }
};
