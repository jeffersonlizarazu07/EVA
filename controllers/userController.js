...
const getUsersByAdmin = async (req, res) => {
  try {
    ...
    if (userType == 2) {
      // Admin: obtener usuarios que compartan clientes
      users = await UserModel.getUsersByClientsAdmin(userId);
    } else {
    ...
    if (!users || users.length === 0) {
      return res.status(404).json({
        ...
        message:
          userType == 2
            ? "No hay otros usuarios con clientes compartidos"
...
const createUser = async (req, res) => {
  ...
  const { firstname, middlename, lastname, type, language, user_red } =
    req.body;
