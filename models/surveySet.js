...
const SurveySet = {
  ...
  copySurvey: async (idOriginalSurvey, link) => {
    if (!idOriginalSurvey || !link) { //!el controlador debe pedir estas 2 cosas en el req.body 
      return false; //!cambiar manejo de errores, retornar en todos los false un return status:false, message:va el error
    }
    try {
      const getSurveyToCopy = await this.getById(idOriginalSurvey);
      console.log(getSurveyToCopy);
  ...
  getByClients: (clientIdsArray) => {
    return db("survey_set").whereIn("idClient", clientIdsArray).select("*");
  },
